...
- [ ] `uv run alembic upgrade head` sobre una base vacía crea las 8 tablas correctamente
- [ ] `uv run alembic downgrade base` revierte la migración inicial sin errores
- [ ] Tras un cambio de prueba en un modelo, `alembic revision --autogenerate` detecta el diff y genera la migración
- [ ] El stack Docker arranca con DB vacía y aplica migraciones solo (sin `create_all`)
- [ ] Los 37 tests de pytest siguen pasando sin modificaciones de lógica
