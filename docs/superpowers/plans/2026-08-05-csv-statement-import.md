...
- [x] **Step 1: Add metadata for `ImportBatch`, per-attempt `ImportRow`, unique `ImportFingerprint`, append-only `TransactionEditEvent`, and transaction `import_batch_id`, `deleted_at`, and `delete_reason`.**
- [x] **Step 2: Add all four tables to the expected application-table set and run `uv run pytest tests/test_migrations.py -v` to confirm metadata needs a migration.**
- [x] **Step 3: Add the reversible Alembic migration.**
...
- [x] **Step 4: Run `uv run pytest tests/test_migrations.py -v`; verify upgrade, downgrade, upgrade, and `alembic check` pass with models and migration aligned.**
- [x] **Step 5: Commit the migration and its test update.**
...
- [x] **Step 1: Write tests proving a soft-deleted transaction disappears from ledger, direct read/mutation, attachments, account balance (including account PATCH), month/budget/range summaries, reports, and client-ID replay. Assert category/account deletion guards remain historically inclusive and direct deletion of imported transactions is rejected to preserve provenance and reversibility.**
- [x] **Step 2: Run the focused tests and confirm they fail with the pre-import model.**
- [x] **Step 3: Add `Transaction.deleted_at.is_(None)` to every normal transaction read, aggregate, attachment, and idempotency lookup. Return 404 for normal reads/mutations of reverted rows, return 409 for direct delete of an imported transaction, and keep category/account reference guards inclusive.**
- [x] **Step 4: Append `TransactionEditEvent` for any material edit to an imported transaction without changing its immutable source snapshot or baseline.**
- [x] **Step 5: Run focused tests, then `uv run pytest`.**
- [x] **Step 6: Commit the model and soft-delete query slice.**
...
- [x] **Step 1: Write preview tests for UTF-8 CSV headers, mapping suggestions, currency normalization, explicit date formats, household direct duplicates, selected-account fingerprints, duplicate file rows, zero amounts, invalid CSV, byte-size/field-size limits, and more than 1,000 rows.**
- [x] **Step 2: Run `uv run pytest tests/test_imports.py -v` and confirm the endpoint is absent.**
- [x] **Step 3: Add camelCase schemas and `POST /import/preview` multipart endpoint.**
...
- [x] **Step 5: Run preview tests and backend lint.**
- [x] **Step 6: Commit preview endpoint, schemas, and tests.**
...
- [x] **Step 2: Run focused commit tests and confirm failure.**
- [x] **Step 3: Implement `POST /import/commit`.**
...
- [x] **Step 4: Run `uv run pytest tests/test_imports.py -v`, then the full backend test suite.**
- [x] **Step 5: Commit the commit endpoint and tests.**
...
- [x] **Step 1: Write failing tests for batch history/detail metadata, created transactions and edit-event visibility, cross-household/personal-account protection, unchanged revert, changed-row conflict, and restoration preserving source-row and transaction IDs.**
- [x] **Step 2: Run the focused tests and confirm failure.**
- [x] **Step 3: Implement `GET /import/batches`, batch detail, `POST /import/{batch_id}/revert`, and `POST /import/{batch_id}/restore`.**
...
- [x] **Step 5: Commit batch lifecycle implementation and tests.**
...
- [x] **Step 1: Add API DTOs and hooks for preview, multipart re-upload commit, batch history/detail, revert, and restore. Invalidate transactions, accounts, month/range summaries, budgets, and batch queries after lifecycle mutations.**
- [x] **Step 2: Add protected `/app/importar` route and an “Importar movimientos” Settings row.**
- [x] **Step 3: Run `npm run lint` and `npm run build` in `frontend`.**
- [x] **Step 4: Commit query/type/routing changes.**
...
- [x] **Step 1: Implement upload step with CSV file selection, accessible account picker, and inline backend errors. Do not submit until both values exist.**
- [x] **Step 2: Implement review step with editable mapping/date format that re-previews the retained file, normalized row table, select-all/select-none, and per-row checkboxes. Duplicate warnings start excluded but can be individually included or excluded; fingerprint rows remain safely skipped by the server. Commit resubmits the retained `File`, never client-normalized values.**
- [x] **Step 3: Implement result step with imported/skipped counts, batch ID, ledger link, and batch-detail/history control.**
- [x] **Step 4: Implement history/detail with source metadata/counts, created transactions, revert confirmation, changed-row conflict display, and restoration action.**
- [x] **Step 5: Check mobile (390px) and desktop layouts, keyboard operation, mapping/date re-preview, global and per-row duplicate selection, invalid-file handling, and revert/restore feedback in the browser.**
- [x] **Step 6: Run `npm run lint` and `npm run build` in `frontend`.**
- [x] **Step 7: Commit the wizard and batch lifecycle UI.**
...
- [x] **Step 1: Run `uv run pytest` from `backend` and record the passing test count.**
- [x] **Step 2: Run `npm run lint` and `npm run build` from `frontend`.**
- [x] **Step 3: Run migration upgrade/downgrade verification against the configured PostgreSQL test workflow.**
- [x] **Step 4: Manually verify real CSV upload, duplicate retry after note edit, batch revert/restore, and account privacy in the browser.**
- [x] **Step 5: Mark roadmap item 08 complete with date and summarize implementation/verification in the roadmap log.**
- [x] **Step 6: Inspect `git status` and `git diff`; commit only feature files.**
